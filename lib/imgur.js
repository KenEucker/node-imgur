...
imgur.updateInfo = function (id, title, description) {
    ...
    imgur
        ._imgurRequest('update', id, extraFormParams)
        .then(function (json) {
            deferred.resolve(json);
        })
        .catch(function (err) {
 ...
 * @param   {string}  query - unique gallery id
 * @returns {promise}
 */
...
imgur.initSearchParams = function (query, options) {
    ...
    for (var key in options) {
        if (key == 'sort' || key == 'dateRange' || key == 'page') {
            params[key] =
                params[key] != options[key] ? options[key] : params[key];
        }
    }
...
imgur.uploadAlbum = function (images, uploadType, failSafe) {
    ...
    if (
        ...
        if (failSafe) {
            ...
            deferred.reject(
                new Error('Invalid image input, only arrays supported')
            );
        }
        return deferred.promise;
    }
