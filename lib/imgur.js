    ...
    imgur._getAuthorizationHeader()
        .then(async (authorizationHeader) => {
            ...
            if (response && !response.success) {
                deferred.reject({status: response.status, message: response.data ? response.error : 'No body data response'});
            } else {
...
imgur.setAccessToken = function (accessToken) {
    if (accessToken && typeof accessToken === 'string') {
        IMGUR_ACCESS_TOKEN = accessToken;
    }
}
 ...
 * @returns {promise}
 */
imgur.updateInfo = function (id, title, description) {
    var deferred = Q.defer(),
        extraFormParams = {};
    ...
    imgur._imgurRequest('update', id, extraFormParams)
        .then(function (json) {
            deferred.resolve(json);
        })
        .catch(function (err) {
 ...
 * @returns {promise}
 */
imgur.getGalleryInfo = function (id) {
    var deferred = Q.defer();

    if (!id) {
        deferred.reject(new Error('Invalid gallery ID'));
        return deferred.promise;
    }

    imgur._imgurRequest('gallery', id)
        .then(function (json) {
            deferred.resolve(json);
        })
        .catch(function (err) {
...
imgur.search = function(query, options) {
    ...
    if(checkQuery.constructor === Error) {
        ...
        imgur._imgurRequest('search', queryStr)
            .then(function (json) {
